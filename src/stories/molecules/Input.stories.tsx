...
import { text, color, boolean, select } from '@storybook/addon-knobs';
...
storiesOf('Molecules', module).add('Input', () => {
  ...
  const inputState = select(
    'State',
    ['error', 'default', 'disabled', 'success'],
    'default',
    COMPONENT_GROUP,
  );

  const placeholderText = text('Placeholder text', '________________', COMPONENT_GROUP);
